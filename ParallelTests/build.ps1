...
function Get-SolutionConfigurations($solution)
{
        ...
        %{ $($Matches['config'])} |
        select -uniq |
        Where-Object {$_ -notmatch "RunInParallel"}
}
...
@(Get-SolutionConfigurations $solution)| foreach {
	Start-Job -ScriptBlock {
        ...
        try
        {
			& $basePath\packages\NUnit.Runners.2.6.4\tools\nunit-console.exe /labels /out=$basePath\nunit_$configuration.txt /xml:$basePath\nunit_$configuration.xml /nologo /config:$configuration "$basePath/ParallelTests/bin/$configuration/ParallelTests.dll"
        }
        finally
        {
            & $basePath\packages\specflow.1.9.0\tools\specflow.exe nunitexecutionreport "$basePath/ParallelTests/ParallelTests.csproj" /out:$basePath\specresult_$configuration.html /xmlTestResult:$basePath\nunit_$configuration.xml /testOutput:nunit_$configuration.txt
        }
